...
def gen_dwdt(tidx):
    if tidx == 0:
        prevw = np.array(netCDF4.Dataset(dy_files[tidx])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
        nextw = np.array(netCDF4.Dataset(dy_files[tidx+1])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
        dwdt = (nextw - prevw) / (60)

    elif tidx == len(dy_files)-1:
        prevw = np.array(netCDF4.Dataset(dy_files[tidx-1])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
        nextw = np.array(netCDF4.Dataset(dy_files[tidx])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
        dwdt = (nextw - prevw) / (60)
    else:
        prevw = np.array(netCDF4.Dataset(dy_files[tidx-1])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
        nextw = np.array(netCDF4.Dataset(dy_files[tidx+1])['w'])[0, :, yargmin:yargmax, xargmin:xargmax]
...
if __name__ == "__main__":
    ...
    xargmin, xargmax = np.argmin(abs(x - 30000)), np.argmin(abs(x - 50000))
    yargmin, yargmax = np.argmin(abs(y - 25000)), np.argmin(abs(y - 45000))
    x = x[xargmin:xargmax]
    y = y[yargmin:yargmax]
    ...
    for tidx in range(init, end):
        ...
        ##### buoyancy #####
        qv = np.array(netCDF4.Dataset(td_files[tidx])['qv'])[0, :, yargmin:yargmax, xargmin:xargmax]
        th = np.array(netCDF4.Dataset(td_files[tidx])['th'])[0, :, yargmin:yargmax, xargmin:xargmax]
        buo = buoyancy(th, qv)
