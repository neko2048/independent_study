...
def draw_core(init, end):
	...
	contourf(np.arange(init, end), z, core_collect, vmin=-0.02, vmax=0.02, levels=10, cmap='coolwarm')
	colorbar()
	title('Core Bouyancy')
	xlabel('Time (min)')
	ylabel('Height (m)')
...
def draw_cloud(init, end):
	...
	contourf(np.arange(init, end), z, cloud_collect, vmin=-0.02, vmax=0.02, levels=10, cmap='coolwarm')
	colorbar()
